...
*Update, Q141 (2026-09-23): the "Z-Anatomy ... its models are CC BY-SA and do not enter this repository" line below reflects the rule as it stood on 2026-09-14 and is kept as written for the historical record. The owner has since decided the anatomy model layer may be CC BY-SA compatible (separate from clinical/imaging layers, which stay proprietary); see `docs/GEOMETRY_SOURCES.md` and `third_party/z-anatomy/README.md`. `data/derived/zanatomy_name_map.json` now reports, per tissue type, how many of the gaps below Z-Anatomy can actually supply.*
...
**Recount 2026-09-19, after Q100** (female viewer V39, male V50): 433 entities, meshes on BOTH bodies for
**202** (unchanged -- both new ids are her-only, still unshipped on him), on at least one for **234** (was 232),
on NEITHER for **199** (was 201, 110 distinct muscles). `upper_limb` missing-on-both dropped 36 -> 34: her right
`extensor_carpi_radialis_longus_r`/`extensor_carpi_radialis_brevis_r`, split by a position rule (not a traced
septum) out of the `extensor_carpi_radialis_compartment` that Q62's original marker-watershed had left merged
(the pale line it found ran on only 67% of shared levels at the weakest ridge ratio in the whole forearm) --
see the Forearm section below and `scripts/cryo/vhf_split_ecrl_ecrb.py`. All other region counts unchanged.
...
`brachioradialis`, `anconeus`, `pronator_teres`, `pronator_quadratus`, `supinator`, `flexor_carpi_radialis`, `flexor_carpi_ulnaris`, `palmaris_longus`, `flexor_digitorum_superficialis`, `flexor_digitorum_profundus`, `flexor_pollicis_longus`, `extensor_carpi_ulnaris`, `extensor_indicis`

`extensor_digitorum`, `extensor_digiti_minimi`, `abductor_pollicis_longus`, `extensor_pollicis_brevis` and `extensor_pollicis_longus` REMOVED from this list (Q97): shipped as `_l` from her own left-forearm cryosection track (Q71's data, which Q71 itself left `atlas_id: null` pending review) -- verified against Q96's bar (watertight, near-single mesh component, 0.0% outside raw skin, within 5-30% of her already-shipped `_r` twin, render-confirmed as the correct dorsal deep-extensor "outcropping" shape). This is a she-only, partial-forearm win, not a precedent for the rest of this list: the remaining 15 left-forearm muscles are proven bad on the SAME dataset (either ~0 cm3, never seeded on the left side, or a 2-10x bilateral mismatch against her shipped right-side twin), a hard fact from Q71's own root-cause diagnosis (the left-forearm bone tracker is proximal-only and absorbs neighbouring muscle territory), not a soft judgment call -- confirmed still correctly declined by Q97.

`extensor_carpi_radialis_longus` and `extensor_carpi_radialis_brevis` REMOVED from this list (Q100): Q62's original marker-watershed had left them merged as one `extensor_carpi_radialis_compartment` (its pale line ran on only 67% of shared levels at ridge ratio 2.06, the weakest boundary in this whole forearm), so they now ship `_r` only, split by a POSITION RULE instead of a traced line -- distance to the already-split, same-volume `extensor_digitorum_r` (ECRL lies radial/superficial, far from it; ECRB lies central/deep, adjacent to it, per Standring's mobile-wad cross-sectional order: brachioradialis-ECRL-ECRB-extensor digitorum), badged `position-rule, not a traced septum` like the abdominal wall's depth-fraction bands. The threshold sits in the range where the actual SHIPPED, smoothed mesh (smooth=1.0; a voxel-connectivity check alone is not enough here -- it found a threshold that ships as two disconnected lobes) stays one connected component for both parts: ECRL 27.1 cm3 (textbook ~20, +35%, mesh 94.6% in one component with a few small satellite islands, comparable to this subject's own already-curated `extensor_carpi_ulnaris_r` at 61%), ECRB 15.9 cm3 (textbook ~15, +6%, mesh 100% one component). `scripts/cryo/vhf_split_ecrl_ecrb.py`. Two of the three merged compartments Q62 left behind (label 1, superficial flexors; label 16, supinator/anconeus) were left untouched -- their own rejection reasons (implausible watershed volumes, a too-small anconeus region) are not position-rule fixable the way label 10's "right compartment, weak internal boundary" problem was. Still missing on him: his forearm has neither name mapped at all.
...
**Q101 (2026-09-19) pursued Q90's own lead (`mentalis`) to a real, verified answer: NOT resolvable by native-resolution colour segmentation, and this closes off the route for the whole group, not just this one muscle.** Located the mandibular symphysis precisely from `vhm_craniofacial_structures.nii.gz`'s mandible label (near-midline anterior projection at RAS z -211 to -248), fetched 54 CONSECUTIVE native-resolution (0.33 mm in-plane) DICOM frames directly from the IDC series (instances 1186-1239, a real craniocaudal range this time, not Q90's 3 spot-check frames), and built a full geometric registration (native cryo pixel -> RAS -> CT voxel, composing `resample_cryo_to_ct_frame.py`'s verified `cryo_idx = -16 - z_ras` mapping with its shift/scale/flip) so the mandible bone mask could be excluded from candidate tissue BY GEOMETRY rather than by colour -- necessary because the mandible's cortex stains tan-brown here, not white, so `cryo_classes.py`'s bone threshold (tuned on the trunk's long bones) does not hold at this location and misclassifies bone as muscle. Found a visually striking, bilaterally symmetric, fanning fibrous-textured darker band immediately outside the projected mandible across ~40 consecutive levels (instances ~1190-1233) -- paired, fan-shaped, a midline notch, right anatomical location: initially a strong-looking candidate. It did not survive quantitative verification. Sampled ground-truth colour from the SAME images: confirmed muscle (tongue/genioglossus) averages r-g=49, v=114 (dark, saturated red); confirmed bone averages r-g=32, v=164 (light); the candidate band averages r-g=33, v=132 -- statistically much closer to bone than to muscle, not a clean muscle-specific signature. A systematic threshold sweep (r-g 28-34, v 140-180) found NO setting giving both a plausible mentalis volume (1-4 cm3 combined both sides, per Standring) AND a single coherent 3D connected component (`scipy.ndimage.label`, 3x3x3 connectivity, across all 54 tracked levels): looser thresholds merge into ONE 9-21 cm3 blob spanning the entire skin+fat+bone-adjacent tissue thickness undifferentiated (3-20x too large to be mentalis alone), tighter thresholds that reach a plausible 6-7 cm3 total fragment into 121-145 disconnected 3D pieces with the two largest capturing only ~77-78% of the total (noise-like speckle, not one coherent muscle body). CONCLUSION: the colour gradient from bone to skin at the chin is smooth with no clean valley anywhere in it, unlike the sharp, saturated, cleanly-bounded colour of confirmed muscle (tongue) in the very same images -- Q90's "subtle, unconfirmed texture patch" does not hold up under proper multi-slice, CT-registered tracking, and is most consistent with ordinary post-mortem subcutaneous staining/vascular variegation, not a resolvable mentalis boundary. Zero muscles shipped -- a real, thorough, honest negative, not a forced ship. **Implication for the rest of this 15-muscle group**: mentalis was this gap's one concrete lead, and (per the task's own framing) one of its LARGER, more distinct members -- a bigger, better-defined target than the wafer-thin orbicularis oris or the widely-spread buccinator. If native-resolution colour segmentation cannot isolate mentalis from surrounding fat, it is very unlikely to do better on the thinner sheet muscles of the rest of the group. This group is now marked **literature-only-for-now** below, the same status as `stapedius`/`tensor_tympani`, not "needs native cryosections" as a pending, fundable route -- unless a fundamentally different acquisition becomes available (histology-grade sectioning, a differently-prepared/stained specimen, or manual expert tracing rather than automated colour segmentation).

Route: **literature-only-for-now (Q101), not a pending native-cryosection route.** `stapedius`/`tensor_tympani` are confirmed out of scope for any photographic or CT source (Q90) and stay literature-only permanently. The other 13 (`occipitofrontalis`, `frontalis`, `orbicularis_oculi`, `procerus`, `nasalis`, `orbicularis_oris`, `buccinator`, `zygomaticus_major`, `zygomaticus_minor`, `levator_labii_superioris`, `depressor_anguli_oris`, `mentalis`, `risorius`) are literature-only for the same reason as of Q101: his head cryosections are streamed and characterized (Q90) and native-resolution frames are fetchable per-slice from the same IDC series (Q101's own method, `scripts/cryo/vhm_stream_crops.py`), but colour/texture segmentation at native resolution failed to isolate even mentalis, the group's best-case candidate -- not for lack of trying at 1mm (Q90) or lack of a real multi-slice native attempt (Q101). A future attempt would need a different technique (manual/expert tracing, a different stain or specimen), not just finer streaming of the same photographs.
