#!/usr/bin/env python3
"""Fast rib voxelization using scipy distance transforms."""
...
def voxelize_structure(
    ...
    """Voxelize and reconstruct mesh using simplified approach."""
    ...
    # Create trimesh but skip voxelized() call
    mesh = trimesh.Trimesh(vertices=consolidated_verts, faces=consolidated_faces, process=False)
    print(f"  Mesh: {len(mesh.vertices)} vertices, {len(mesh.faces)} faces")

    # Manual voxelization: get bounds and fill using signed distance
    print(f"  Voxelizing at {voxel_size}mm resolution...")
    v_min = mesh.vertices.min(axis=0)
    v_max = mesh.vertices.max(axis=0)

    grid_shape = np.ceil((v_max - v_min) / voxel_size).astype(int) + 1
    print(f"  Grid shape: {grid_shape}, voxels: {np.prod(grid_shape):,.0f}")

    # Initialize voxel grid as empty
    voxel_grid = np.zeros(grid_shape, dtype=np.uint8)

    # Simple approach: scan along z-axis and mark voxels
    print("  Scanning for occupied voxels...")
    for i in range(grid_shape[0]):
        if i % 20 == 0:
            print(f"    X: {i}/{grid_shape[0]}")
        for j in range(grid_shape[1]):
            for k in range(grid_shape[2]):
                voxel_center = np.array([i, j, k]) * voxel_size + v_min
                # Check if point is inside mesh
                # Simple heuristic: check distance to nearest face
                dist, _ = mesh.nearest.distance_to_point(voxel_center.reshape(1, 3))
                if dist < voxel_size:
                    voxel_grid[i, j, k] = 1
    ...
    dilated = ndimage.binary_dilation(voxel_grid > 0, structure=struct, iterations=dilation_iterations)
