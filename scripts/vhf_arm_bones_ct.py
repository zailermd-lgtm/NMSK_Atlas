"""Upper-limb bones of the VH FEMALE fresh CT (torso block) by the male's marker-controlled watershed
(segment_arm_bones_vhm.py), adapted 2026-09-12: her arms are clipped laterally by the 480 mm field of view, so
what comes out is partial (Q12 found a partial right radius and ulna); each side runs independently and a side
without two forearm fragments is reported, not asserted. Output vhf_ts/arm_bones_ct.nii.gz on her torso grid.

Male description follows.
...
im=nib.load(S+"vh_idc/nii/vhf_torso_0937.nii.gz"); ct=np.asarray(im.dataobj); seg=np.asarray(nib.load("/home/user/NMSK_Atlas/data/ct_sources/task_outputs/vhf_total.nii.gz").dataobj)
inv={v:int(k) for k,v in json.load(open("/home/user/NMSK_Atlas/mappings/totalsegmentator_labels.json"))["labels"].items()}
...
for side,(x0,x1) in (("right",(0,150)),("left",(362,512))):
  try:
    ...
    print(side,"FAILED:",repr(ex)[:200],flush=True); report[f"{side}_failed"]=repr(ex)[:200]
nib.save(nib.Nifti1Image(out,im.affine),S+"vhf_ts/arm_bones_ct.nii.gz"); json.dump(report,open(S+"vhf_ts/arm_bones_ct_report.json","w"),indent=1)
lut=np.array([[0,0,0],[200,200,200],[255,80,80],[80,160,255],[255,220,60],[200,200,200],[255,120,200],[90,230,120],[240,160,60]],np.uint8)
for side,(x0,x1) in (("right",(0,150)),("left",(362,512))):
    if not (out[x0:x1]>0).any(): continue
    for ax,nm in ((1,"cor"),(0,"sag")):
        p=out[x0:x1,:,:].max(axis=ax); rgb=lut[p]; base=np.clip((ct[x0:x1,:,:].max(axis=ax)+200)/1400,0,1)*140
        img=np.where(p[...,None]>0,rgb,np.repeat(base[...,None],3,2)).astype(np.uint8)
        Image.fromarray(img.transpose(1,0,2)[::-1]).save(S+f"vhf_ts/arm_ct_{side}_{nm}.png")
