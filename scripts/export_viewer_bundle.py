...
def summarise(folder, rec, anchor_points=None):
    ...
    out = {
        ...
        "notes": rec.get("notes"),
        # Q119: standing project convention -- ANY entity record whose mesh
        # geometry is procedural/synthetic (generated by a script, not
        # segmented from imaging) must carry a top-level `procedural_geometry`
        # object with at least a `badge` string (the full, honest disclosure
        # text: what is generated, what -- if anything -- is still a real
        # measurement, and a pointer to the generating script/PROJECT_STATE.md
        # entry). Established by Q104's intervertebral discs (pure procedural
        # cylinders) and Q118's tendon connectors (measured length + modeled
        # radius); both were retrofitted onto this field by Q119 after finding
        # neither disclosure reached the published viewer. Only `badge` is
        # forwarded into the bundle -- the rest of the object (verification
        # numbers, per-subject measurements) is for the entity record/git
        # history, not the inspector panel. The viewer template renders this
        # as a `.tag.warn` chip, the same visual pattern already used for
        # cross-subject-transferred structures (search "tag warn" there).
        "procedural_badge": (rec.get("procedural_geometry") or {}).get("badge"),
    }
