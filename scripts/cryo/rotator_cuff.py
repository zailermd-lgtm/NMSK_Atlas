...
for side,(cols,ids) in {"right":(slice(0,360),LAB["right"]),"left":(slice(340,700),LAB["left"])}.items():
    ...
    for k in range(z0,z1+1):
        ...
        if ribs.any():
            drib=ndi.distance_transform_edt(~ribs); m&=drib>8     # serratus / intercostals hug the ribs; the cuff does not
            m&=~((dist>18)&(drib<dist))                        # v2: ventral voxels nearer the ribs than the scapula are serratus, and subscapularis stays within 18 mm
