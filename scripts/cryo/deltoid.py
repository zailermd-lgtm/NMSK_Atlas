...
for side,(cols,hum,lid,sgn) in {"right":(slice(0,300),1,1,+1),"left":(slice(400,700),5,2,-1)}.items():
    ...
    for k in range(max(0,ktop-150),min(n,ktop+21)):
        ...
        if lat_edge is not None:
            xx=np.arange(cols.stop-cols.start)[None,:]; m&=((xx<=lat_edge+8) if sgn>0 else (xx>=lat_edge-8)) | (((xx<=lat_edge+35) if sgn>0 else (xx>=lat_edge-35))&(dskin<=25))   # v2: posterior deltoid over the lateral third of the spine, superficial only
