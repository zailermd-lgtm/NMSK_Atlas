...
for k in range(0,ztop):
    ...
    rect=wall&(np.abs(xx-mid)<=70)&(yy<=A+45)   # v2: 45 mm behind the anterior skin
    ...
    if lat.any():
        ...
        if lat.any():
            ...
            for side,sgn,base in (("r",1,2),("l",-1,6)):
                ...
                for lab_,cond in ((base,frac<0.40),(base+1,(frac>=0.40)&(frac<0.75)),(base+2,frac>=0.75)):
                    mm_=sm&cond
                    if mm_.any():
                        cl2,n2=ndi.label(mm_); sz=ndi.sum(np.ones_like(cl2),cl2,np.arange(1,n2+1)); mm_=cl2==(np.argmax(sz)+1)   # v2: one contiguous piece per layer per slice
                    o[mm_]=lab_
